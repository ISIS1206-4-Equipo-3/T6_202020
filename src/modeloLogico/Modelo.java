...
public class Modelo {
	...
	try {
		...
		tabla = new BST<Date, ArrayList>(null);
		...
		while(((linea = lector.readNext())!=null)){
			if(linea != null ) {
				...
				if(tabla.get(fechaInicial) == null)
				{
					ArrayList<Accidente> accidentes = new ArrayList<>();
					accidentes.add(accidente);
					tabla.put(fechaInicial, accidentes);
				}
				else
				{
					ArrayList valores = (ArrayList) tabla.get(fechaInicial);
					valores.add(accidente);
					tabla.put(fechaInicial, valores);					
				}
		...
		Accidente minimo = (Accidente) ((ArrayList) tabla.get(tabla.min())).get(0);
		minimo.imprimirAccidente();
		System.out.println("Valor maximo");
		Accidente maximo = (Accidente) ((ArrayList) tabla.get(tabla.max())).get(((ArrayList) tabla.get(tabla.max())).size()-1);
